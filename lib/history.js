 ...
 * `runId#attempt`. Call order is therefore: reconciled (weakest), stored, own
 * run (strongest) - the local context always knows more about the current run
 * than the API does, which still reports it `in_progress`.
 */
function mergeEntries(...lists) {
  ...
  for (const list of lists) {
    for (const entry of list || []) {
      ...
      byKey.set(key, previous ? {...previous, ...next, m: next.m || previous.m} : next);
    }
  }
...
function renderGroup(group, serverUrl, repoSlug, isNewest) {
  const rows = group.runs.map(run => {
    const glyph = GLYPHS[run.s] || '❔';
    const logs = run.r ? `[run](${serverUrl}/${repoSlug}/actions/runs/${run.r})` : '—';
    return `| ${cell(run.c)} | ${glyph} ${cell(run.s)} | ${formatStarted(run.at)} | `
      + `${formatDuration(run.d)} | ${run.a || 1} | ${cell(run.by)} | ${logs} |`;
  });
...
function renderTable(entries, {serverUrl, repoSlug, trimmed}) {
  const groups = groupByCommit(entries);
  const parts = [HEADING, ''];
  groups.forEach((group, i) => {
    parts.push(renderGroup(group, serverUrl, repoSlug, i === 0));
  });
  if (trimmed) {
    parts.push('');
...
function spliceBlock(body, block) {
  ...
  const separatorIndex = withoutOrphan.indexOf(AI_SEPARATOR);
  if (separatorIndex >= 0) {
