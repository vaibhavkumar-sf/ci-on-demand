...
function ownEntry({context, checkName, status, startedAt, now, headSha = ''}) {
  ...
  return {
    ...
    c: checkName,
    s: status,
    // `headSha` when the caller knows better than the run does - a check
    // dispatched at the default branch has that branch's tip in `context.sha`,
    // and a row filed under it would group under a commit the PR never had.
    sha: headSha || context.payload.pull_request?.head?.sha || context.sha,
...
async function reconcileEntries({github, context, core, branch}) {
  ...
  for (const run of data.workflow_runs || []) {
    ...
    entries.push({
      ...
      // The workflow's display name is not the status context; flag it so a row
      // that already carries the real context keeps it.
      w: 1,
    });
  }
...
async function updateRunHistory({
  ...
  const now = new Date();
  let mine = ownEntry({context, checkName, status, startedAt, now, headSha});
  ...
  for (let attempt = 1; attempt <= 2; attempt++) {
    ...
    let entries = mergeEntries(stored, reconciled, [mine]);
